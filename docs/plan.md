...
6. ✅ **5-rep × 3 sequences** (full determinism protocol, done for all systems). ATE-trans (evo se3): V1_01 0.030, MH_03 0.061, V2_02 0.049 m — paper-ballpark. Basalt: 100% completeness, ~0 s init, lightest RAM (~90 MB), most parallel (~13 cores), and deterministic (ATE std ±0.000).
...
**Current deliverable:** [`docs/comparison.md`](/home/hailo/workspace/vio-evaluation/docs/comparison.md) — auto-generated by `compare_report.py` covering **OpenVINS, ORB-SLAM3 (SLAM + VIO-only), and Basalt** across V1_01/MH_03/V2_02 at the **full 5-rep protocol** (mean±std; OpenVINS/Basalt deterministic ±0.000, ORB-SLAM3 shows multi-thread variance), with the DR §3.1 metric table, §2.6 per-segment RPE tables, and a data-driven Conclusions section (each bullet cites the DR + checks its targets). Accuracy uses **evo** (one engine for all systems). 5-rep mean ATE-t ranking: ORB-SLAM3 0.021 < Basalt 0.047 < OpenVINS 0.066 m. SchurVINS drops in as a 4th system once Phase 3 is unblocked (the report + conclusions regenerate to include it automatically).
...
### Phase 5 — UZH-FPV drone dataset (2nd dataset axis) 🔶 IN PROGRESS

Beyond the original EuRoC scope: extend the harness to the [UZH-FPV Drone Racing Dataset](https://fpv.ifi.uzh.ch/) — aggressive quadrotor flight, **fisheye** (Snapdragon stereo) + IMU. Kept as a **separate** comparison from EuRoC (different camera model + difficulty regime), tag `uzhfpv_x86`. Full details in [`docs/uzhfpv.md`](/home/hailo/workspace/vio-evaluation/docs/uzhfpv.md).

Key differences from EuRoC (drive the extra work): (1) **fisheye KB4** calibration — each rig (`indoor_45_*`, `indoor_forward_*`, `outdoor_forward_*`) has its own Kalibr camchain, EuRoC calib does not apply; (2) distributed as **ROS 1 `.bag`** (`_with_gt` sequences only — GT from Leica MS50); (3) outdoor GT is partial → report coverage% alongside ATE.
...
**Remaining steps to complete Phase 5** (in order):

1. **Finish the ORB-SLAM3 UZH leg on `indoor_45_2`** — run the full 5-rep protocol for each variant (stereo-inertial, stereo `--vio-only`, and monocular-inertial `--mono`) via `run_orb_slam3_uzh.sh`; confirm ATE/RPE against the bag-extracted GT (evo se3). Sanity-check the fisheye `KannalaBrandt8` YAML by eyeballing the trajectory vs GT before trusting numbers.
2. **Stage the other UZH sequences** — `indoor_forward_3`, `indoor_forward_7`, `outdoor_forward_1` (`_with_gt`). Each is a separate download → `uzh_bag_to_asl.py` + `uzh_gt_to_tum.py`. **Per-rig calib**: `indoor_forward_*` and `outdoor_forward_*` need their own `UZH-FPV.yaml` from their own Kalibr camchains (regenerate via `uzh_calib_to_orbslam3.py` / `uzh_calib_to_basalt.py`).
3. **Run ORB-SLAM3 + Basalt** (5-rep) across all staged UZH sequences under tag `uzhfpv_x86`.
4. **UZH comparison table in `compare_report.py`** — add a UZH path/mode that aggregates `~/results/<system>/x86/native_jazzy/uzhfpv_x86/` (ORB-SLAM3 SLAM/VIO-only/mono + Basalt), emits a *separate* UZH table (the fisheye/aggressive regime isn't comparable to EuRoC), and reports **coverage %** next to ATE for partial-GT outdoor sequences. Render into `docs/uzhfpv.md` (or a UZH section of `comparison.md`).
5. **OpenVINS on UZH** — needs a UZH fisheye `estimator_config` + camchain, and the UZH **ROS 1** bag fed to OpenVINS' reader (our `run_openvins.sh` uses the ROS 2 `serial.launch`; either convert the UZH bag ROS 1→ROS 2 with `rosbags-convert`, or use OpenVINS' ROS 1 serial path). Treat as its own sub-task.
6. **SchurVINS on UZH** — blocked by the same missing-container-runtime issue as Phase 3; resume once a runtime exists.
7. **Commit + housekeeping** — commit the ORB-SLAM3 UZH leg (`run_orb_slam3_uzh.sh`, `uzh_calib_to_orbslam3.py`, generated YAMLs, submodule bump for the `mono_inertial_euroc` build) and `publish_dataset_previews.sh`; update the `docs/uzhfpv.md` system table (still marks ORB-SLAM3 "pending").
...
- **Additional EuRoC sequences** beyond V1_01/MH_03/V2_02 — still deferred (the three match OpenVINS cross-platform.md). Note: a *different* dataset axis (UZH-FPV) is now an active extension — see **Phase 5** above, not this item.
