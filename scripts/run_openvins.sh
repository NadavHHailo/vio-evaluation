...
usage() { echo "usage: run_openvins.sh <seq> [--tag TAG] [--thr N]"; exit 1; }
[ $# -ge 1 ] || usage
SEQ="$1"; shift
TAG="baseline_x86"; THR=4
while [ $# -gt 0 ]; do
  case "$1" in
    --tag) TAG="$2"; shift 2;;
    --thr) THR="$2"; shift 2;;
...
echo "[OV $SEQ] serial, threads=$THR; out=$OUT"
...
/usr/bin/time -v -o /tmp/vioeval_ov_time.log \
  ...
  > "$OUT/${SEQ}_stdout.log" 2>&1

[ -f "$EST_TMP" ] || { echo "ERROR: no estimate produced (see $OUT/${SEQ}_stdout.log)" >&2; exit 1; }
cp "$EST_TMP" "$OUT/${SEQ}_est.txt"
cp "$TIMING_WALL_TMP" "$OUT/${SEQ}_wall.txt"
cp "$TIMING_CPU_TMP"  "$OUT/${SEQ}_cpu.txt"
awk '
...
' /tmp/vioeval_ov_time.log > "$OUT/${SEQ}_proc.csv"

echo "[OV $SEQ] done: $(grep -cv '^#' "$OUT/${SEQ}_est.txt") poses; $(cat "$OUT/${SEQ}_proc.csv" | tail -1)"
